...
getwd()
data = read.csv("TelecomCustomerChurn.csv")
head(data)
...
# Confusion matrix
conf_matrix <- table(Predicted = logistic_pred_class, Actual = testData$Churn)
print(conf_matrix)
...
# ROC and AUC
roc_curve <- roc(testData$Churn, logistic_pred)
...
# Confusion matrix
tree_conf_matrix <- table(Predicted = tree_pred, Actual = testData$Churn)
